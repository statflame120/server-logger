...
public class ScrollableList extends Widget {
    ...
    public static class ListItem {
        ...
        public final int color; // 0 = use theme default
        public final Object data; // optional associated data
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float delta) {
        ...
        // Smooth scroll lerp (skip when dragging scrollbar)
        ...
        for (int i = startIdx; i < endIdx; i++) {
            ...
            if (urlMatcher.find()) {
                // Render with clickable URL highlighting
    ...
    private void renderWithUrls(GuiGraphics g, String text, int textX, int textY,
        ...
        while (matcher.find()) {
            ...
            // Render URL with accent color + underline
            ...
            int urlColor = urlHover ? 0xFFFFFFFF : accentColor;
    ...
    public boolean onMouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (button == 0) {
            ...
            if (maxScroll > 0 && mouseX >= scrollTrackX && mouseX < scrollTrackX + 5) {
                ...
                targetScrollOffset = scrollOffset; // Sync target to prevent lerp fight
                autoScroll = false;
                return true;
            }
    ...
    public boolean onMouseScrolled(double mouseX, double mouseY, double hAmount, double vAmount) {
        if (!visible || !containsPoint(mouseX, mouseY)) return false;
        if (maxScroll > 0) {
            targetScrollOffset -= vAmount * ITEM_HEIGHT * 2;
            targetScrollOffset = Math.max(0, Math.min(targetScrollOffset, maxScroll));
            autoScroll = false;
            return true;
        }
